...
import {
  ...
  type WallpaperRatingItem,
  type WallpaperReportItem,
} from "../api";
...
export default function WallpaperReview() {
  ...
  useEffect(() => {
    ...
    loadRatings(selectedWallpaper.id);
  }, [selectedWallpaper]);
  ...
  const removeRating = async (id: number) => {
    if (!selectedWallpaper) return;
    try {
      const res = await wallpaperAdmin.deleteRating(id, selectedWallpaper.id);
      if (res.code === 200) {
        showMsg("评分删除成功");
        await loadRatings(selectedWallpaper.id);
        await loadWallpapers();
      } else {
  ...
  return (
    <div style={pagePaddingStyle}>
      ...
      <div style={{ ...cardStyle, marginBottom: 16 }}>
        <h2 style={{ margin: "0 0 12px", color: "#fff", fontSize: 20 }}>举报处理</h2>
      ...
      <div style={cardStyle}>
        <h2 style={{ margin: "0 0 12px", color: "#fff", fontSize: 20 }}>评分管理</h2>
        {!selectedWallpaper ? (
